...
export default function TemplateDetailScreen() {
  ...
  const loadTemplate = async () => {
    try {
      ...
      console.log(
        `✅ Datos actualizados recibidos: ${data.name} - ${data.sets.length} sets, ${data.sets.reduce((acc: number, set: any) => acc + set.exercises.length, 0)} ejercicios`
      );
      setTemplate(data);
        ...
        <View style={styles.setsSection}>
          ...
          {template.sets.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>Esta rutina no tiene sets configurados</Text>
            </View>
          ) : (

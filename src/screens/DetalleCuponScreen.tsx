...
const buildImageUrl = (obj: any): string => {
  ...
  if (typeof direct === "string" && direct.length) {
    return /^https?:\/\//i.test(direct) ? toHttpsIfSurtek(direct) : `${API_BASE}/${direct.replace(/^\/+/, "")}`;
  }
...
/* API claim */
type ClaimResponse = { code?: string; qr_url?: string; data?: any; qr?: { code?: string; url?: string } };
async function claimQr(promotionId: string | number, payload: any) {
  ...
  return { code: json.code ?? json.qr?.code ?? json.data?.code, qr_url: json.qr_url ?? json.qr?.url ?? json.data?.qr_url };
}
    ...
    <View style={[style, { justifyContent: "center", alignItems: "center", overflow: "hidden" }]}>
      ...
      {error && (
        <View
          style={[
            style,
            { position: "absolute", top: 0, left: 0, justifyContent: "center", alignItems: "center", backgroundColor: "#f5f5f5" },
          ]}
        >
...
export default function DetalleCuponScreen({ route }: any) {
  ...
  const paramObj = params.beneficio ?? params.cupon ?? params.item ?? params.promotion ?? params.promo ?? null;
  const [cupon] = useState<UiCupon | null>(paramObj ? (paramObj.titulo ? fromBeneficio(paramObj) : fromApi(paramObj)) : null);
  ...
  const safe = useMemo<UiCupon>(
    () => cupon ?? ({ id: "", titulo: "", descripcion: "", imagenUrl: "", validoInicio: "", validoFin: "", terms: "", raw: null }),
    [cupon]
  );
  ...
  const handleClaim = async () => {
    ...
    if (user_id == null) {
      setClaimError("Necesitás iniciar sesión para reclamar este cupón (user_id numérico requerido).");
      return;
    }
  ...
  const lineaValidez =
    safe.validoInicio && safe.validoFin ? `${safe.validoInicio} — ${safe.validoFin}` : safe.validoInicio || safe.validoFin || "—";
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {claiming ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Obtener Cupón</Text>}
      </TouchableOpacity>

      {!!mustLogin && <Text style={styles.infoText}>Iniciá sesión para canjear (user_id numérico requerido).</Text>}
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {!!claimed?.qr_url && <Text style={styles.qrNote}>También tenés un enlace asociado al QR.</Text>}
...
const styles = StyleSheet.create({
  ...
  image: { width: "100%", height: 180, borderRadius: 10, marginBottom: 16, backgroundColor: "#f5f5f5" },
  ...
  button: { backgroundColor: "#4CAF50", paddingHorizontal: 20, paddingVertical: 12, borderRadius: 10, marginTop: 4 },
  ...
  closeButton: { marginTop: 16, backgroundColor: "#f44336", paddingHorizontal: 15, paddingVertical: 8, borderRadius: 8 },
